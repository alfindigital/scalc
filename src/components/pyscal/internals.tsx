...
export function focusGridCell(row, col) {
  ...
  if (el && typeof (el as any).focus === 'function') {
    (el as any).focus();
    if (typeof (el as any).select === 'function') { try { (el as any).select(); } catch {} }
    return true;
  }
  return false;
}
...
export function handleSetupEnter(e) {
  ...
  if (next) {
    e.preventDefault();
    (next as any).focus();
    if (typeof (next as any).select === 'function') { try { (next as any).select(); } catch {} }
  }
}

export function BidStepInput({ value, onChange, onFocus, disabled, className = '', variant = 'desktop', label, warning, error, gridRow, gridCol, ...rest }) {
...
export function LotInput({ value, onChange, className = '', label, gridRow, gridCol, onKeyDown, ...rest }) {
  const reactId = useId();
